...
from .forms import ProductForm,LoginForm,ProductUpdateForm,OrderUpdateForm
...
def display_orders(request):
    if request.method == 'POST':
        ...
        delivered_status = request.POST.get('delivered')

        if order_id and delivered_status in ['True', 'False']:
            order = Order.objects.get(id=order_id)
            order.delivered = delivered_status == 'True'
            order.save()
    ...
    context = {'orders': orders, 'count': count_orders}
